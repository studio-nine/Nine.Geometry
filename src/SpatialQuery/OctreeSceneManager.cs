namespace Nine.Geometry.SpatialQuery
{
    ...
    {
        /// <summary>
        /// Gets the underlying octree tree.
        /// </summary>
        /// <remarks>
        /// Only for advanced use.
        /// </remarks>
        public Octree<List<ISpatialQueryable>> Tree { get; private set; }
        ...
        private readonly Func<OctreeNode<List<ISpatialQueryable>>, TraverseOptions> add;
        ...
        private readonly Func<OctreeNode<List<ISpatialQueryable>>, TraverseOptions> findAllRay;
        private readonly Func<OctreeNode<List<ISpatialQueryable>>, TraverseOptions> findAllBoundingBox;
        private readonly Func<OctreeNode<List<ISpatialQueryable>>, TraverseOptions> findAllBoundingSphere;
        private readonly Func<OctreeNode<List<ISpatialQueryable>>, TraverseOptions> findAllBoundingFrustum;
        ...
        private TraverseOptions Add(OctreeNode<List<ISpatialQueryable>> node)
        {
            ...
            containment = node.bounds.Contains(itemBounds);
        ...
        private TraverseOptions FindAllRay(OctreeNode<List<ISpatialQueryable>> node)
        {
            ...
            if (skip)
            {
                intersection = node.bounds.Intersects(ray);
        ...
        {
            var nodeContainment = boundingBox.Contains(node.bounds);
        ...
        {
            var nodeContainment = boundingSphere.Contains(node.bounds);
            if (nodeContainment == ContainmentType.Disjoint)
                return TraverseOptions.Skip;
        ...
        {
            var nodeContainment = boundingFrustum.Contains(node.bounds);
            if (nodeContainment == ContainmentType.Disjoint)
                return TraverseOptions.Skip;
            ...
            if (node.value != null)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    nodeContainment = boundingFrustum.Contains(boundingBox);
                    if (nodeContainment != ContainmentType.Disjoint)
                    {
                        result.Add(val);
                    }
